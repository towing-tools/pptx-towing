...
function validateTextBoxPosition(slideData, bodyDimensions) {
  ...
  const minBottomMargin = 0.1; // Relaxed from 0.5 to 0.1 inches
...
async function html2pptx(htmlFile, pres, options = {}) {
  ...
  try {
    ...
    if (validationErrors.length > 0) {
      ...
      console.warn(`[VALIDATION WARNING] ${htmlFile}: ${errorMessage}`);
      // throw new Error(errorMessage); // コメントアウトして中断を回避
    }
